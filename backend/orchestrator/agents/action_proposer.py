...
async def action_proposer_node(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    try:
        ...
        system_prompt = (
            ...
            "1. 'create_goal': Requires parameters {'description': str, 'project_id': Optional[str]}\n"
            "2. 'update_goal_status': Requires parameters {'goal_id': str, 'status': 'active'|'completed'|'archived'|'paused'}\n"
            "3. 'create_project': Requires parameters {'space_id': str, 'name': str}\n"
            "4. 'update_project_status': Requires parameters {'project_id': str, 'status': 'active'|'completed'|'archived'|'on_hold'}\n"
            "5. 'add_memory': Requires parameters {'content': str, 'memory_type': str, 'importance': 'high'|'medium'|'low'}\n\n"
            ...
            "4. If a recommendation is general advice or unsupported (e.g. 'create a task', 'send an email', 'delete a file'), return NO proposal for that recommendation.\n"
